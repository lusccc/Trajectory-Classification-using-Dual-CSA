...
class Trajectory_Feature_Dataset(Dataset):
    def __init__(self, dataset_name, data_type):
        ...
        self.pedcc = torch.from_numpy(np.load(f'./data/{self.dataset_name}_features/pedcc.npy'))
    ...
    def __getitem__(self, index):
        RP = self.transform(torch.from_numpy(self.multi_channel_RP_mats[index]))
        label = self.labels[index]
        FS = self.multi_feature_segs[index]
        return RP, self.pedcc, FS, label
...
def calc_RP_data_mean_std(dataset_name, data_type, use_gpu=False):
    ...
    # data = dataset.RP_mats_ndarray.to(device)
    ...
    print("Std:", std)
    print("calc_RP_data_mean_std elapsed time: %.3f seconds" % (timeit.time.perf_counter() - start))
    return mean, std
