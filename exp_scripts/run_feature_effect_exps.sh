...
for i in $(seq 0 1 8); do
  path="./results/${dataset}_feature${features[${i}]}_exp0"
  mkdir -p ${path}
...
#  python network_training.py --dataset ${dataset} --results-path ${path} --alpha 1 --beta 11 --gamma 1 --RP-emb-dim 152 --FS-emb-dim 152 --patience 20 --dist-url tcp://127.0.0.1:6666 --dist-backend nccl --multiprocessing-distributed --world-size 1 --rank 0 -b 330 --n-features ${n_features} --visualize-emb 0
  python network_training.py --dataset ${dataset} --results-path ${path}  --RP-emb-dim 152 --FS-emb-dim 152 --patience 20 --dist-url tcp://127.0.0.1:6666 --dist-backend nccl --multiprocessing-distributed  --world-size 1 --rank 0 -b 405 --n-features ${n_features}
