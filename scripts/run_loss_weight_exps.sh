...
for i in $(seq 0 1 5); do
  path="./results/exp1st_SHL_t7s3e136a${alpha[${i}]}b${beta[${i}]}y${gamma[${i}]}/"
  mkdir -p ${path}
  ...
  python -m network.Dual_SAE --dataset SHL --results_path ${path} --alpha ${alpha[${i}]} --beta ${beta[${i}]} --gamma ${gamma[${i}]} --RP_emb_dim 68 --ts_emb_dim 68
