...
from trj_features_and_segmentation import MAX_SEGMENT_SIZE
...
def dual_SAE():
    ...
    concat_embedding = concatenate([RP_conv_ae.get_layer('RP_embedding').output,
                                    lstm_ae.get_layer('spatial_embedding').output])
    centroids_input = Input((N_CLASS, TOTAL_EMBEDDING_DIM))
    classification = classification_layer([concat_embedding, centroids_input])

    d_sae = Model(
        inputs=[RP_conv_ae.get_layer(index=0).input, centroids_input, lstm_ae.get_layer(index=0).input],
        outputs=[RP_conv_ae.get_layer('RP_reconstruction').output, classification, lstm_ae.get_layer('spatial_reconstruction').output])
    d_sae.summary()
    plot_model(d_sae, to_file='./results/dual_sae.png', show_shapes=True)
    d_sae.compile(loss=['mse', 'kld', 'mse'], loss_weights=[1, 1, 1], optimizer='adam',
